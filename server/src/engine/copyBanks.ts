...
function hooks(brief: Brief): string {
  const topic = brief.product.split('.')[0]!.slice(0, 60);
  const aud = brief.audience || 'founders';
  return POWER_HOOKS.map((h, i) =>
    `${i + 1}. ${h.replace('{topic}', topic).replace('{audience}', aud)}`,
  ).join('\n');
}

function paragraphs(brief: Brief, blocks: string[]): string {
...
function vslScript(brief: Brief): string {
...
function landingPage(brief: Brief): string {
  return paragraphs(brief, [
...
function emailSequence(brief: Brief, count: number): string {
  const subjects = [
...
function genericBlock(brief: Brief, label: string): string {
  return paragraphs(brief, [
    `## ${label}`,
...
export function buildAssets(funnel: FunnelType, brief: Brief): GeneratedAsset[] {
  ...
  return meta.assets.map(({ key, label }) => {
    ...
    else if (key === 'landing_page' || key === 'sales_page' || key === 'optin_page') content = landingPage(brief);
    else if (key === 'email_sequence') content = emailSequence(brief, funnel === 'lead_magnet' ? 5 : 3);
    else content = genericBlock(brief, label);
...
export function buildBrief(product: string, audience: string, tone: string): Brief {
