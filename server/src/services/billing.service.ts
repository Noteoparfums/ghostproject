...
import type { BillingInterval, SubscriptionStatus } from '@ghostwriter/shared';
import { randomUUID } from 'node:crypto';
import { withTransaction, type TransactionConnection } from '../lib/db.js';
...
export const billingService = {
  async getActivePlan(userId: number, tx?: TransactionConnection) {
  ...
  async createSubscriptionCheckout(userId: number, planSlug: string, interval: BillingInterval) {
  ...
  async createTopupCheckout(userId: number, amountCents: number) {
    const user = await userRepository.findById(userId);
    if (!user) throw new AppError('NOT_FOUND', 'User not found');
  ...
  async cancelSubscription(userId: number) {
    const sub = await subscriptionRepository.findActiveByUserId(userId);
    if (!sub) throw new AppError('VALIDATION_ERROR', 'No active subscription found');
  ...
  async handleWebhookEvent(provider: string, eventId: string, type: string, payload: any): Promise<any> {
      ...
      await withTransaction(async (tx) => {
        if (type === 'checkout.session.completed') {
          ...
          if (mode === 'subscription') {
            ...
            const plan = await planRepository.findBySlug(planSlug, tx);
            if (!plan) throw new Error(`Plan slug ${planSlug} not found in database`);
          ...
          if (providerSubId) {
            const sub = await subscriptionRepository.findByProviderId(providerSubId, tx);
            if (sub) {
              const currentAttempts = (sub as any).dunning_attempts || 0;
          ...
          if (sub) {
            ...
            if (status === 'paused') targetStatus = 'paused';
            else if (status === 'active') targetStatus = 'active';
            else if (status === 'past_due') targetStatus = 'past_due';
            else if (status === 'unpaid') targetStatus = 'expired';
            else if (status === 'canceled') targetStatus = 'cancelled';
