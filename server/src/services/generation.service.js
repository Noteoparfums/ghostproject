import { generationRepository, generationAssetRepository, generationStageRepository } from '../repositories/generation.repository.js';
import { creditService } from './credit.service.js';
import { sseRegistry, openSse } from '../lib/sse.js';
...
export const generationService = {
    async startGeneration(userId, payload, res) {
        ...
        const gen = await withTransaction(async (tx) => {
            ...
            await creditService.charge(userId, cost, 'generation', { generationId: generation.id }, `Charged ${cost} credits for funnel generation`);
            return generation;
        });
    ...
    async runGenerationPipeline(genId, userId, payload, handle, signal) {
    ...
    async regenerateSection(userId, assetId, instruction) {
        ...
        const balance = await creditService.balance(userId);
        if (balance < cost)
            throw new AppError('PAYMENT_REQUIRED', 'Insufficient credits for regeneration');
