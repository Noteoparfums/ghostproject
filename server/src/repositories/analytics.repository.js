...
export const analyticsRepository = {
    async getSystemHealth(tx) {
       ...
       WHERE created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)`, [], tx);
        return {
    ...
    async getRecentErrorDigests(limit = 10, tx) {
        return query('SELECT * FROM error_digests ORDER BY last_seen DESC LIMIT ?', [limit], tx);
    },
    async getJobRuns(limit = 10, tx) {
        return query('SELECT * FROM job_runs ORDER BY started_at DESC LIMIT ?', [limit], tx);
    }
};
