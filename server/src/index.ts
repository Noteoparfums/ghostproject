...
async function assertMigrationsCurrent() {
  ...
  try {
    const row = await queryOne<{ count: number }>('SELECT COUNT(*) as count FROM schema_migrations');
...
async function assertDemoLedgerConsistency() {
  try {
    const demoUser = await queryOne<{ id: number }>(
...
function registerGracefulShutdown(server: Server, scheduler: any) {
    ...
    try {
      ...
      if (activeIds.length > 0) {
        ...
        for (const genId of activeIds) {
          try {
            const gen = await queryOne<{ user_id: number; credits_charged: string }>(
              'SELECT user_id, credits_charged FROM generations WHERE id = ?',
              [genId]
            );
            if (gen) {
              ...
              try {
                await withTransaction(async (conn) => {
                  await execute(
                    "UPDATE generations SET status = 'failed', error_message = 'Server restart' WHERE id = ?",
                    [genId],
                    conn
                  );
                  ...
                  if (cost > 0) {
                    ...
                    await execute(
                      "INSERT INTO credit_ledger (user_id, delta, balance_after, source, generation_id, note) VALUES (?, ?, ?, 'refund', ?, ?)",
                      [userId, cost, after, genId, 'Refund for in-flight generation aborted by server restart'],
                      conn
                    );
                  }
                });
