 ...
 */
export function isValidEuVatId(vatId: string, countryCode?: string): boolean {
...
export function countryFromVatId(vatId: string): string | null {
  const normalized = normalizeVatId(vatId);
  const prefix = normalized.slice(0, 2);
  if (!EU_VAT_PATTERNS[prefix]) return null;
